...
<<match2, fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
...
<<match3,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
...
<<matchj,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
brj2$partsaved <- brj2$part
brj2$part <- (brj2$partsaved - 3) %% 6 + 1
qplot(y, resid, data=subset(brj1, x>=50 & x <=55), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_point(aes(y, resid), colour="black", size=1, alpha=0.2, 
             data = subset(brj2, x>=50 & x <=55)) + 
...
<<match5,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
brj2$partsaved <- brj2$part
brj2$part <- (brj2$partsaved - 3) %% 6 + 1
qplot(y, resid, data=subset(brj1, x>=50 & x <=55), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
...
<<match6,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
brj2$partsaved <- brj2$part
...
<<match7,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
brj2$partsaved <- brj2$part
brj2$part <- (brj2$partsaved - 2) %% 6 + 1
qplot(y, resid, data=subset(brj1, x>=50 & x <=55), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
...
<<match10,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
...
<<match10ortho,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
qplot(y, oresid, data=subset(br101, x>=50 & x <=55), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
...
@
<<matchsmooth,  fig.width=8, fig.height=6, echo=FALSE, warning=FALSE>>=
...
#dsdb1 <- subset(dsdb1, y >= 150 & y <= 850)
#dsdb2 <- subset(dsdb2, y >= 150 & y <= 850)
