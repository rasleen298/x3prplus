...
The command {\tt fortify.x3p} (our function) allows us to get a data set of a grid of equi-spaced $x$ and $y$ values and scanned values:
<<data, message=FALSE>>=
require(x3pr)
require(x3prplus)
br111 <- read.x3p(paste(datadir,"Br1 Bullet 1-1.x3p", sep="/"))
dbr111 <- fortify.x3p(br111)
head(na.omit(dbr111))
@
...
for (i in 1:6) {
  ...
  dbi <- fortify.x3p(read.x3p(bname))
  dbi$part <- i
  db1 <- rbind(db1, dbi)
}
...
db1$theta <- with(db1, acos((y-x0)/radius)/pi*180)

db1$xpred <- with(db1, x0+radius*cos(theta/180*pi))
db1$ypred <- with(db1, y0+radius*sin(theta/180*pi))
@
...
db2$theta <- with(db2, acos((y-x0)/radius)/pi*180)

db2$xpred <- with(db2, x0+radius*cos(theta/180*pi))
db2$ypred <- with(db2, y0+radius*sin(theta/180*pi))
@

First attempt to match - something is wrong with bullet piece 3.

<<echo=FALSE, warning=FALSE>>=
qplot(y, value-ypred, data=subset(db2, x <= 55 & x >=50), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_point(aes(y, value-ypred), colour="black", size=1, alpha=0.2, 
             data = filter(db1, part==1, x <= 55, x >=50)[,c("y", "ypred", "value")]) + ggtitle("Part 1 of Bullet 1 in black")
@
...
qplot(y, value-ypred, data=subset(db2, x <= 55 & x >=50), colour=factor(x)) + 
  ...
  theme_bw() + theme(legend.position="bottom") + 
  geom_point(aes(y, value-ypred), colour="black", size=1, alpha=0.2, 
             data = filter(db1, part==3, x <= 55, x >=50)[,c("y", "ypred", "value")]) + ggtitle("Part 3 of Bullet 1 in black")
@
...
<<echo=FALSE, warning=FALSE>>=
db1$part <- ((db1$part + 1) %% 6) +1
qplot(y, value-ypred, data=subset(db2, x <= 55 & x >=50), colour=factor(x)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_point(aes(y, value-ypred), colour="black", size=1, alpha=0.2, 
             data = filter(db1, x <= 55, x >=50)) + ggtitle("Attempt at an overall match of Bullets 1 and 2")
@
...
qplot(y, value-smy, data=sdb2, colour=factor(x)) + 
  ...
  geom_point(aes(y, value-smy), colour="black", size=1, alpha=0.2, 
             data = sdb1) + ggtitle("Attempt at an overall match of Bullets 1 and 2 based on a nonparametric smooth")
@
